...
```{r exampleObject1}
example.object <- dataframeToExperiment(dataframe =  example,
                                        timeIndex = 1)
...
The main functions are *runTestGraph()* and *analyzeExperiemnt()*. You can call *analyzerExperiment()* on you experiment object to get physiologically relivant data, including:
- Beats per minute
- Amplitute
...
write.csv(example.object, "~/Desktop/results.csv")
```
